...
Table longest_common_seq(double X[], double Y[], int size_X, int size_Y){
    ...
    table.length.resize((size_X + 1) * (size_Y + 1)); // wierd for this line!
    ...
    for(int i = 1; i <= size_X; i++){
        for(int j = 1; j <= size_Y; j++){
            if(X[i-1] == Y[j-1]){
                table.length[j * (size_X+1)+i] >= table.length[(j-1) * (size_X+1) + i];
                table.arrow[j * (size_X+1)+i] = 0;  // "up-left arrow"
            }
            else if(table.length[(j-1) * (size_X+1) + i] >= table.length[j * (size_X+1) + (i-1)]){
                table.length[j * (size_X+1) + i] = table.length[(j-1) * (size_X+1) + i];
                table.arrow[j * (size_X+1) + i] = 1;  // "up arrow"
            }
            else{
                table.length[j * (size_X+1) + i] = table.length[j * (size_X+1) + (i-1)];
...
void print_LCS(const Table& table, double X[], int i, int j){
    ...
    if(table.arrow[j * (table.length.size() / (i+1)) + j] == 0){
        print_LCS(table, X, i-1, j-1);
        cout << X[i-1] << " ";
    }
    else if(table.arrow[j * (table.length.size() / (j+1)) + i] == 1){
